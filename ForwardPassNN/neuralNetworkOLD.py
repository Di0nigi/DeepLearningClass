...
def GlogLoss(y, p):
    epsilon = 1e-15
    p = np.clip(p, epsilon, 1 - epsilon)  # Clip p to prevent division by zero
    return (-y / p + (1 - y) / (1 - p)) * p * (1 - p)
...
class FeedForwardNN:
    ...
    def backP(self,pred,vanilla,labels):
        ...
        for ind,elem in enumerate(pred):
            ...
            g=GlogLoss(elem,labels[ind])#(labels[ind]-elem)*2
            mg+=g
            gradients.append(g)
